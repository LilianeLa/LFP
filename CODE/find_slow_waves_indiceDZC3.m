...
function find_slow_waves_indiceDZC3(V)
global def b red t		
			...
			if ((iUZC-iDZC) <= 	(def.SWlength(2)*def.rate/1000) && ...
				...
				if valmin <= def.SWmAmpl(1)					% disp('Amplitude of negative peak is more negative than def.SWmAmpl(1)')					
					...
					if upperboundPicPositif > 0
						...
						elseif 	   ((indiceDZC+2)<size(DZC,2)) && (DZC(indiceDZC+2)-iUZC < def.rate*def.SWlength(3)/1000)		% We examine between iUZC and the DZC after the next DZC (we want to find the maximum peak)
						...
						elseif ((indiceDZC+1)<size(DZC,2)) && (DZC(indiceDZC+1)-iUZC < def.rate*def.SWlength(3)/1000)			% If the DZC after the next DZC is too far (i.e. is above duration threshold),
							[valmax, indposmax]= max(F1(iUZC:DZC(indiceDZC+1)));												% then we examine between iUZC and the next DZC (we want to find the maximum peak)					
						end	
