...
% The algorithm computes the sum of these booleans, simply corresponding to the scalar 'countBP'. 
% For example, in the case there are 10 OA and 'countBP'=10, it means each OA has a BP. 
...
% Moreover, all the random windows are different, such as there won't be twice the same random window 
% and there won't be any redundant slow waves detected.
...
function find_BP_P300_randSW(vertical, SW, V)				% The input is in fact ttl.times but we can't write "function find_BP(ttl.times)" although we can write "find_BP(ttl.times)"	in the prompt
global def			
...
if countrandSW > 1 
	disp([' - ', num2str(countrandSW), ' of the ', num2str(length(oa)), ' random windows show at least one SW (which is neither BP or P3).'])
elseif countrandSW == 1 	
	disp([' - 1 of the ', num2str(length(oa)), ' random windows shows at least one SW (which is neither BP or P3).'])
else 
	disp([' - 0 of the ', num2str(length(oa)), ' random windows shows at least one SW.'])
end	
disp('*********************************************************************************************************');
disp(' ');
